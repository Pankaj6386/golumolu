...
const VideoTypes = ['ChatVideo', 'CommentVideo', 'FeedVideo', 'ReminderVideo']
const getVideoUrl = (type, isWithoutTranscode) => {
    let videoType
    if (!type) {
        videoType = 'ChatVideo'
    ...
    if (isWithoutTranscode) {
        return `${API_URL_DEV}secure/s3/${videoType}/signature`
    }
    return `${API_URL_DEV}secure/s3/${videoType}/transcode`
    // return `http:/192.168.1.12:8081/api/secure/s3/${videoType}/signature`
}

const VideoUtils = {
    getPresignedUrl(file, token, dispatch, type, videoRef) {
        return new Promise(async (resolve, reject) => {
            ...
            axios
                ...
                .then((res) => {
                    const { s3_url, s3_folder, message, success } = res.data
                    console.log(
                        'objectKey',
                        message,
                        s3_url,
                    ...
                    resolve({ message, s3_url, s3_folder, success })
                })
                .catch((err) => {
    ...
    getPresignedUrlWithoutTranscode(file, token, dispatch, type) {
        return new Promise(async (resolve, reject) => {
            const url = getVideoUrl(type, true)
