...
import PostSearchOverlay from './Main/Search/PostSearchOverlay'
...
//nudge
import NudgeListViewAll from './Main/Notification/Nudge/NudgeListViewAll'

/* Registration */
// import RegistrationAccount from './Registration/Account';
// import RegistrationAccount from './Registration/RegistrationAccount';
import ReminderDashboard from './Main/Goal/NewGoal/ReminderDashboard'
import {
    RegistrationAccount,
...
import Deactivate from './Main/Setting/Account/Deavtivate'
import { color } from './styles/basic'
import PasswordReset from './PasswordReset'
import QRCodeScreen from './Main/Tribe/QRCodeScreen'
import GoogleSignUp from './Main/Onboarding/GoogleSignUp'
// Remainder feature
import ArchiveScreen from './Main/Goal/NewGoal/ArchiveScreen'
import CreateReminder from './Main/Goal/NewGoal/CreateReminder'
import { refreshActivityFeed } from './redux/modules/home/feed/actions'
import { refreshNotificationTab } from './redux/modules/notification/NotificationTabActions'
import { getAllNudges } from './actions/NudgeActions'
import NotificationToogle from './Main/Setting/Account/Notifications/NotificationToogle'
import ReminderTribe from './Main/Goal/NewGoal/ReminderTribe'
import ReminderContacts from './Main/Goal/NewGoal/ReminderContacts'
import ReminderFriends from './Main/Goal/NewGoal/ReminderFriends'
import ReminderTribeDetails from './Main/Goal/NewGoal/ReminderTribeDetails'
...
function getCommonScenes(tab) {
    ...
    return [
        ...
        <Scene key={`${prefix}deactivate`} component={Deactivate} />,
        ...
        />,
        <Scene
            key={`${prefix}notification_toogle`}
            component={NotificationToogle}
        />,
        ...
        <Scene
            key="registration_contact_invite_from_friend"
...
class RouterComponent extends Component {
    ...
    async componentDidMount() {
        if (Platform.OS === 'ios') {
            Linking.getInitialURL()
                .then((res) => {
                    console.log('Quit State Event: 1 ', res)
                    this.handleUniversalLink(res)
                })
                .catch((err) => console.log('err in Deeplinks', err))

            Linking.addEventListener('url', (link) => {
                console.log('Foreground/Background Event: ', link)
                this.handleUniversalLink(link)
            })
        } else {
            dynamicLinks().onLink((link) => this.handleUniversalLink(link))
            dynamicLinks()
                .getInitialLink()
                .then((link) => this.handleUniversalLink(link))
        }
    }

    async handleUniversalLink(link) {
        console.log('Deeplink URL: ', link)
        const url = link.url ? link?.url : link
        if (url) {
            const destRoute_0 = url.replace('https://web.goalmogul.com/', '')
            const destRoute = destRoute_0.replace(
                'https://goalmogul.page.link/',
                ''
            )

            const key = destRoute.split('/')[0]

            const path = destRoute.split('/')[1]

            //uncomment if wanted to reload user current scene with new props if deeplink & current scene are same
            // if(key === Actions.currentScene){
            // Actions.refresh()
            // }else{/////
            this.deeplinkNavigation(key, path)
            // }
        }
    }

    deeplinkNavigation(key, path) {
        console.log('Deep Navigation: ', key + ' :: ' + path)

        if (key === 'creategoal') {
            Actions.createGoalModal({ funnel: path })
            return
        }

        if (key === 'silverchallenge') {
            Actions.silverchallenge()
            return
        }
        ...
        if (key === 'chat') {
            Actions.jump('chatTab')
            Actions.chat({ funnel: path })
            return
        }

        if (key === 'invitefriend') {
            Actions.invitefriend({ funnel: path })
            return
        }
        if (key === 'tribehub') {
            Actions.jump('exploreTab')
            Actions.tribehub({ funnel: path })
            return
        }
    ...
    onTabPress = (all) => {
        ...
        if (state.key === 'notificationTab') {
            this.props.dispatch(refreshNotificationTab())
            this.props.dispatch(getAllNudges())
        ...
        if (state.key === 'homeTab' && isFocused()) {
            this.props.dispatch(refreshActivityFeed())
                ...
                <Modal key="modal" hideNavBar>
                    <Lightbox key="lightbox" hideNavBar>
                        ...
                        >
                            ...
                            <Scene
                                key="ReminderTribe"
                                component={ReminderTribe}
                                hideNavBar
                            />
                            <Scene
                                key="ReminderTribeDetail"
                                component={ReminderTribeDetails}
                                hideNavBar
                            />
                            <Scene
                                key="ReminderFriends"
                                component={ReminderFriends}
                                hideNavBar
                            />
                            <Scene
                                key="ReminderContacts"
                                component={ReminderContacts}
                                hideNavBar
                            />
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <Scene
                                                key="NudgeListViewAll"
                                                component={NudgeListViewAll}
                                                hideNavBar
                                            />
                                            <Scene
                                        ...
                                        >
                                            ...
                                            <Scene
                                                path={
                                                    '/chatRoomConversation/:chatRoomId'
                                                }
                    ...
                    <Stack key="qrCodeStack" hideNavBar>
                        <Scene
                            key="qrCodeScreen"
                    ...
                    />
                    <Scene
                        key="searchPostLightBox"
                        component={PostSearchOverlay}
                        hideNavBar
                    />
                    <Scene
