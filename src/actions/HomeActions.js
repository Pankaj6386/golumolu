...
export const lastActive = () => {
  ...
  return async (dispatch, getState) => {
    const token = await TokenService.getAuthToken();
    const url = `https://goalmogul-api-dev.herokuapp.com/api/secure/user/survey/lastactive`;
    const headers = {
      method: "POST",
      headers: {
        Accept: "application/json",
    ...
    fetch(url, headers)
      ...
      .then((res) => {
        ...
        if (res.message) {
          /* TODO: error handling */
          console.log(`${DEBUG_KEY}: error in last active api: `, res);
          return;
        }
        console.log(`${DEBUG_KEY}: Last Active Api successful: `, res);
      ...
      /* TODO: error handling */
      .catch((err) =>
        console.log(`${DEBUG_KEY}: exception in last active:`, err)
      );
  };
};
