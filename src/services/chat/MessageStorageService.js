...
class MessageStorageService {
    ...
    /**
     * Updated reminder status locally
     * @param {string} reminderID
     * @param {Function} callback: (err, storedDoc)
     */
    updateReminderStatus = (reminderID, callback) => {
        localDb.update(
            {
                'reminderRef._id': reminderID,
            },
            {
                $set: {
                    'reminderRef.reminder_status': 'completed',
                },
            },
            {
                multi: true,
    ...
    /**
     * Updated reminder request status locally
     * @param {string} reminderID
     * @param {string} userId
     * @param {number} status
     * @param {Function} callback: (err, storedDoc)
     */
    updateReminderRequestStatus = (reminderID, userId,status, callback) => {
        localDb
            .find({
                'reminderRef._id': reminderID,
            })
            .exec((err, result) => {
                if (err) {
                    console.error('error in find reminder', err)
                    return callback && callback(err, null)
                }
                const respArray = Array.isArray(result) ? result : []
                const allMessagedId = []
                let updatedRecipientUser
                for (let i = 0; i < respArray.length; i++) {
                    const message = respArray[i]
                    const id = message._id
                    allMessagedId.push(id)
                    ...
                    const recipientUsers = Array.isArray(
                        ...
                        ? reminder.recipient_users
                        : []
                    updatedRecipientUser = recipientUsers.map((item) => {
                ...
                if (updatedRecipientUser?.length) {
                    localDb.update(
                        {
                            _id: {
                                $in: allMessagedId,
                        ...
                        {
                            $set: {
                                'reminderRef.recipient_users': updatedRecipientUser,
                            },
                        },
                        {
                            multi: true,
