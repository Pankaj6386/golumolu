...
import { REMINDER_REQUEST_STATUS } from '../../../Utils/Constants'
...
export const CHAT_ROOM_UPDATE_REMINDER_STATUS_MESSAGES =
    'chat_room_update_reminder_status_messages' // we update the reminder status when user click button on mark as done
export const CHAT_ROOM_UPDATE_REMINDER_REQUEST_STATUS_MESSAGES = // we update the reminder request status when user click on accept button
...
export default (state = INITIAL_STATE, action) => {
    switch (action.type) {
        ...
        case CHAT_ROOM_UPDATE_REMINDER_STATUS_MESSAGES: {
            console.log('CHAT_ROOM_UPDATE_REMINDER_STATUS_MESSAGES')
            let newState = _.cloneDeep(state)
            ...
            if (reminderId) {
                ...
                for (let i = 0; i < oldMessages.length; i++) {
                    ...
                    if (message?.reminderRef?._id === reminderId) {
                        message.reminderRef.reminder_status = 'completed'
                    }
                    updatedMessages.push(message)
                }
                return _.set(newState, `messages`, updatedMessages)
            }
            return newState
        }
        case CHAT_ROOM_UPDATE_REMINDER_REQUEST_STATUS_MESSAGES: {
            let newState = _.cloneDeep(state)
            ...
            if (reminderId) {
                ...
                for (let i = 0; i < oldMessages.length; i++) {
                    const message = oldMessages[i]
                    if (message?.reminderRef?._id === reminderId) {
                        const reminder = message?.reminderRef
                        const recipientUsers = Array.isArray(
                            reminder.recipient_users
                        )
                            ? reminder.recipient_users
                            : []
                        message.reminderRef.recipient_users = recipientUsers.map(
                            (item) => {
