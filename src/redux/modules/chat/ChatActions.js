...
export const getGomoPlannerRoomRef = () => (dispatch, getState) => {
    const state = getState()
    const { token } = state.user

    fetchGomoPlannerChatRoom(token)
        .then((res) => {
            console.log('gomoplanner Ref: ', res)
            if (res.data)
                dispatch({
                    type: CHAT_ROOM_GOMO_PLANNER_REF,
        ...
        .catch((err) => {
            console.log('error getting gomo planner room ref', err)
            dispatch({
                type: CHAT_ROOM_GOMO_PLANNER_REF,
                payload: null,
            })
...
async function fetchGomoPlannerChatRoom(token) {
    try {
        return await API.get(`secure/chat/room/chatbot/gomo_planner`, token)
    } catch (e) {
        /* best attempt */
    }
}
async function fetchCoachRexChatRoom(token) {
    try {
        return await API.get(`secure/chat/room/chatbot/coach_Rex`, token)
    } catch (e) {
        /* best attempt */
    }
}
