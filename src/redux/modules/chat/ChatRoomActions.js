...
export const getChatVideoProgress = async (socket) => {
    socket.on('Chat Video Transcoding', async (data) => {
        console.log('socket data chat response===>', data)
        store.dispatch(
            setChatVideoProgress({
        ...
        if (data.progress === 100) {
            socket.on('Chat Video Uploading', async (data) => {
                console.log('socket data chat uploading response===>', data)
                if (data.progress === 100) {
                    store.dispatch(
                        backToInitialStateForChatVideo({
                            progress: 100,
