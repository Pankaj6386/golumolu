...
const searchWithId = (searchContent, queryId, searchType) => (
    ...
    fetchData(
        ...
        (res) => {
            ...
            const isSuggested = res.suggested ? true : false
            console.log('RESPONSE WAS 2', isSuggested)
...
export const searchUser = (searchContent, skip, limit, callback) => async (
    ...
    const { token, userId } = getState().user
    console.log('SEARCHH CONTENTT', searchContent)
    ...
    (searchType) => (dispatch, getState) => {
        ...
        fetchData(
            ...
            (res) => {
                const data = res.data ? res.data : []
                const isSuggested = res.suggested ? true : false
                console.log('RESPONSE WAS 1', isSuggested)
                dispatch({
...
const SuggestedSearchRouteMap = {
    people: {
        route: `${BASE_ROUTE}/user/profile/atlas`,
    },
    tribes: {
        route: `${BASE_ROUTE}/Tribe/atlas`,
    },
    chatRooms: {
        route: `${BASE_ROUTE}/chat/room/atlas`,
    },
    post: {
        route: `${BASE_ROUTE}/feed/post/atlas`,
    },
}
const fetchData = curry(
    (searchContent, type, skip, limit, token, callback, forceRefresh) => {
        ...
        const suggestedBaseRoute = _.get(SuggestedSearchRouteMap, type)
        ...
        API.get(
            ...
            .then((res) => {
                console.log(
                    `${DEBUG_KEY} fetching with res: `,
                    res,
                    suggestedBaseRoute?.route,
                    type
                )
                if (res.data && callback && res.data.length) {
                    console.log('firebase', res)
                    callback(res)
                    return
                } else if (suggestedBaseRoute) {
                    // Retry suggested search:
                    API.get(
                        `${suggestedBaseRoute.route}?query=${searchContent}`,
                        token
                    ).then((res) => {
                        if (res.data && callback) {
                            callback({ ...res, suggested: true })
                            return
                        }
                        callback({ data: [] })
                    })
...
export const refreshPreloadData = (type) => (dispatch, getState) => {
    ...
    API.get(route, token)
        .then((res) => {
            console.log('THIS IS PRELOAD DATA', res, route)
