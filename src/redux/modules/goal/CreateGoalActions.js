...
export const submitGoal = (
    ...
    const onSuccess = (goalDetails) => {
        ...
        if (
            isStoreReminder &&
            isCustomReminder &&
            Actions.currentScene !== 'CreateReminder'
        ) {
            dispatch(setSelectedGoal(goalDetails))
            Actions.push('CreateReminder')
        }
        if (
            isStoreReminder &&
            !isCustomReminder &&
            Actions.currentScene !== 'ReminderDashboard'
        ) {
            dispatch(setReminderSelectedTabIndex(0))
            Actions.push('ReminderDashboard')
        } else if (!isStoreReminder) {
            setTimeout(() => {
    ...
    API.post(
        ...
        .then(async (res) => {
            if (res.status === 200 || (res.data && !_.isEmpty(res.data))) {
                ...
                callback(res.data)
                if (isStoreReminder && !isCustomReminder) {
                    await dispatch(createNewReminder(res?.data?._id))
