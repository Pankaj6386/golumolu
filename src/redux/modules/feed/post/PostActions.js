...
import { EMPTY_POST, PRIVACY_PUBLIC } from '../../../../Utils/Constants'
...
export const submitCreatingPost = (
    ...
    if (!needUpload) {
        ...
        VideoUtils.getPresignedUrl(
            ...
            .then((video) => {
                // Use the presignedUrl as media string
                console.log('media ref after uploading is: ', video)

                return sendCreatePostRequest(
                    { ...newPost, mediaRef: video },
                    token,
                    dispatch,
                    onSuccess,
                    null,
...
export const getPostVideoProgress = async (socket) => {
    socket.on('Feed Video Transcoding', async (data) => {
        console.log('socket data feed response===>', data)
        store.dispatch(isPostVideoTranscoded(true))
        store.dispatch(
            setFeedVideoProgress({
        ...
        if (data.progress === 100) {
            socket.on('Feed Video Uploading', async (data) => {
                console.log('socket data feed uploading response===>', data)
                if (data.progress === 100) {
                    store.dispatch(
                        backToInitialStateForFeedVideo({
                            progress: 100,
                    ...
                    store.dispatch(isPostVideoTranscoded(false))
                }
            })
        }
    })
