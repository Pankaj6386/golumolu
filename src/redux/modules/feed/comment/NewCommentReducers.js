...
export const COMMENT_NEW_SELECT_VIDEO = 'comment_new_select_video'
...
export default (state = INITIAL_STATE, action) => {
    switch (action.type) {
        ...
        case COMMENT_NEW_SELECT_IMAGE: {
            const { tab, mediaRef, pageId, type } = action.payload
            const newState = _.cloneDeep(state)
            const page = pageId ? `${pageId}` : 'default'
            const path = !tab ? `homeTab.${page}` : `${tab}.${page}`
            return (
                _.set(newState, `${path}.mediaRef`, mediaRef),
                _.set(newState, `${path}.type`, type)
            )
        }
        case COMMENT_NEW_SELECT_VIDEO: {
            const { tab, mediaRef, pageId, type } = action.payload
            const newState = _.cloneDeep(state)
            const page = pageId ? `${pageId}` : 'default'
            const path = !tab ? `homeTab.${page}` : `${tab}.${page}`
