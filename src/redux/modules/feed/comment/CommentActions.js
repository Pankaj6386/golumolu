...
import { switchCase, sanitizeTags } from '../../../middleware/utils'
 ...
 */
export const newCommentOnMediaRefChange = (mediaRef, pageId, type) => (
    dispatch,
    getState
) => {
    const { tab } = getState().navigation
...
export const newCommentOnMediaRefChangeVideo = (mediaRef, pageId, type) => (
    dispatch,
    getState
) => {
    const { tab } = getState().navigation
    dispatch({
        type: COMMENT_NEW_SELECT_VIDEO,
        payload: {
            mediaRef,
            tab,
            pageId,
 ...
 */
export const postComment = (pageId, callback, generatedReply) => (
    dispatch,
    getState
) => {
    ...
    ImageUtils.getImageSize(mediaRef)
        ...
        .then((res) => {
            ...
            console.log('newCommentObject 1', newCommentObject)
        ...
        if (callback) {
            ...
            setTimeout(() => {
                DropDownHolder.alert(
                    ...
                    `We'll notify you when it becomes available. Thanks for your patience!`
                )
            }, 500)
        } else {
            dispatch({
...
export const getVideoProgress = async (socket) => {
    socket.on('Comment Video Transcoding', async (data) => {
        console.log('socket data response===>', data)
        store.dispatch(
            setVideoProgress({
                progress: data.progress,
                uploaded: true,
                uploadedFor: data.for,
                userId: data.userId,
                videoUploading: false,
            })
        )
        if (data.progress === 100) {
            socket.on('Comment Video Uploading', async (data) => {
                console.log('socket data uploading response===>', data)
                if (data.progress === 100) {
                    store.dispatch(
                        backToInitialStateForVideo({
                            progress: 100,
    ...
    API.post(
        `${BASE_ROUTE}`,
        {
            comment: JSON.stringify(
                generatedReply !== undefined ? changedContext : newComment
            ),
        },
        token
    )
...
export const createSuggestion = (goalId, pageId) => (dispatch, getState) => {
    ...
    const newComment = _.get(getState().newComment, `${path}`)
    const suggestion = newComment?.suggesstion
    const tmpSuggestion = newComment?.tmpSuggestion
...
export const refreshComments = (
    ...
    const localComment = _.get(getState().comment, path)
    const hasNextPage = localComment?.hasNextPage
