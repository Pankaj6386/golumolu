...
import {
    ...
    REGISTRATION_UPDATE_GENDER_LOADING,
    REGISTRATION_UPDATE_GENDER_ERROR,
    REGISTRATION_UPDATE_GENDER_SUCCESS,
} from '../../../actions/types'
...
export const registerAccount = (onSuccess) => async (dispatch, getState) => {
    ...
    try {
        ...
        console.error(err)
        trackWithProperties(E.REG_ACCOUNT_FAILED, {
            error: err.message,
        })
...
export const updateBasicProfile = (payload) => async (dispatch, getState) => {
    try {
        dispatch({ type: REGISTRATION_UPDATE_GENDER_LOADING, payload: true })
        const userData = await API.put(
            'secure/user/account/basic_profile',
            payload
        )
        if (userData.status === 200) {
            await Auth.removeNotHaveGender()
            dispatch({ type: REGISTRATION_UPDATE_GENDER_SUCCESS })
            return userData
        }
        dispatch({ type: REGISTRATION_UPDATE_GENDER_LOADING, payload: false })
    } catch (e) {
        dispatch({ type: REGISTRATION_UPDATE_GENDER_ERROR, payload: e.message })
        console.error(DEBUG_KEY, 'Error in update profile', e)
        throw e
    }
}
