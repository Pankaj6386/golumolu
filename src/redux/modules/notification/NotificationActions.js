...
import { setHandleDueDateNotification } from '../../../reducers/ReminderFormReducers'
...
export const subscribeNotification = () => async (dispatch, getState) => {
    ...
    if (existingStatus !== 'granted') {
        ...
        finalStatus = status
        console.log('Request permission', finalStatus)
    ...
    const { status: existingStatuss } = await checkNotifications()
    ...
    if (Platform.OS === 'ios') {
        const tkn = await messaging().getAPNSToken()
        notificationToken = { type: Platform.OS, data: tkn }
    } else {
        notificationToken = await Notifications.getDevicePushTokenAsync()
    }

    // Alert.alert(JSON.stringify(notificationToken))
    ...
    const onSuccess = async (res) => {
        ...
        ReactMoE.passFcmPushToken(notificationToken.data)
