...
class MyTribe extends React.PureComponent {
                    ...
                    this.setState({ tooltipVisible: false }),
                        setTimeout(() => {
                            Actions.push('myTribeGoalInviteFriends', {
                                ...
                                pageId: this.props.pageId,
                                tags: {
                                    member: this.props.item.members
                                        .filter(
                                            (doc) => doc.category === 'Member'
                                        )
                                        .map((i) => i.memberRef._id),
                                    requested: this.props.item.members
                                        .filter(
                                            (doc) =>
                                                doc.category === 'JoinRequester'
                                        )
                                        .map((i) => i.memberRef._id),
                                    invited: this.props.item.members
                                        .filter(
                                            (doc) => doc.category === 'Invitee'
                                        )
                                        .map((i) => i.memberRef._id),
                                    admin: this.props.item.members
                                        .filter(
                                            (doc) => doc.category === 'Admin'
                                        )
                                        .map((i) => i.memberRef._id),
                                },
                            })
    ...
    makeTribeOptions = (item) => {
        ...
        const qrCodeOption = {
            text: 'QRCode',
            textStyle: { color: 'black' },
            icon: { name: 'qrcode', pack: 'material-community' },
            onPress: () => {
                let admin = this.props.item.members
                    .filter((doc) => doc.category === 'Admin')
                    .map((i) => i.memberRef)
                let tribe = this.props?.item

                this.closeOptionModal()
                // Wait for bottom sheet to close
                // before showing unfriend alert confirmation
                setTimeout(() => {
                    Actions.push('qrCodeStack', { admin, tribe })
                }, 500)
            },
        }

        if (isAdmin) {
            return [qrCodeOption, editOption, deleteOption]
        }
        if (isMember) {
                ...
                onPress: () =>
                    Actions.push('myTribeGoalInviteFriends', {
                        ...
                        tags: {
                            ...
                            requested: item.members
                                .filter(
                                    (doc) => doc.category === 'JoinRequester'
                                )
                                .map((i) => i.memberRef._id),
                            invited: item.members
                                .filter((doc) => doc.category === 'Invitee')
