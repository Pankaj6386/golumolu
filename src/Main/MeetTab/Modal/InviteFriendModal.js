...
import InviteFriendTooltip from '../../../components/InviteFriendTooltip'
...
const DESCRIPTIONS_TOP3 = [
    ...
    `GoalMogul is an exciting new goal-sharing community for possibility thinkers. We can use its planner to create empowering goals, then stay connected by helping each other achieve them. I’d love to get your feedback on my goals and find out more about yours too! 🙏 Please join me:`,
...
class InviteFriendModal extends React.PureComponent {
    ...
    getInviteLink = () => {
        if (this.props.isTribe) {
            return generateInvitationLink(
                this.props.tribeCode?.tribeInviteCode?.code
            )
        } else {
    ...
    postOnTwitter = (message, url) => {
        const shareOptions = {
            message,
            url,
            social: ShareRN.Social.TWITTER,
        }
        console.log('shareOptions', shareOptions)
        ShareRN.shareSingle(shareOptions)
            .then((res) => {
                console.log('TWITTER OPENED', res)
            })
            .catch(() => {
                setTimeout(() => {
                    Alert.alert('Error!', 'App is not installed!')
    ...
    shareOnMessanger = async (message, url) => {
        const shareOptions = {
            message,
            url,
            social: ShareRN.Social.MESSENGER,
        }
        ShareRN.shareSingle(shareOptions)
            .then((data) => {
    ...
    inviteEmail = async (message, url) => {
        ...
        if (canOpen) {
            Linking.openURL(
                `mailto:?subject=I’m looking for some help or advice with my new goals 🙏
                &body=${unescape(
                    `${message} Use this Invite Code to sign up: ${this.state.inviteCode}`
                )}\n\n${unescape(url)}`
            )
    ...
    shareToWhatsApp = async (message, url) => {
        const shareOptions = {
            message,
            url,
            social: ShareRN.Social.WHATSAPP,
        }
        await ShareRN.shareSingle(shareOptions).catch((e) => {
            console.log(e)
        })
    }
