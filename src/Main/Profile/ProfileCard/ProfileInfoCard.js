...
import LinkIcon from '../../../asset/image/linkIcon.png'
...
class ProfileInfoCard extends Component {
    ...
    renderFriendInfo() {
        const title = this.props.canEdit ? 'Friends' : 'Mututal Friends'
    ...
    renderAbout(about, helpOrServe, likeToConnect) {
        if (
            (about && about.trim().length > 0) ||
            (helpOrServe && helpOrServe.trim().length > 0) ||
            (likeToConnect && likeToConnect.trim().length > 0)
        ) {
            return (
                <View style={{ alignSelf: 'flex-start' }}>
                    <View
                        style={[
                            profileStyles.subHeaderStyle,
                            styles.paddingHorizontalView,
                            styles.paddingVerticalView,
                        ]}
                    >
                    ...
                    {helpOrServe?.trim() ? (
                        <View>
                            <Text
                                style={[
                                    PROFILE_STYLES.subAboutInfoTitle,
                                    { marginBottom: 8 },
                                    styles.paddingHorizontalView,
                                ]}
                            >
                        ...
                        if (item) {
                            return (
                                ...
                                >
                                    ...
                                    />
                                    <Text style={styles.linkStyle}>{item}</Text>
                                </TouchableOpacity>
                            )
                        }
                        return null
    ...
    render() {
        ...
        const { likeToConnect, helpOrServe, links } = user
        const detailsCard = (
            <View style={styles.containerStyle}>
                {this.renderAbout(about, helpOrServe, likeToConnect)}
        ...
        return (
            <View>
                <View style={[styles.containerStyle, styles.paddingView]}>
                    {/* {this.renderAllFriendInfo()} */}
