...
import { copilot, CopilotStep, walkthroughable } from 'react-native-copilot-gm'
...
import SuggestedNeedModal from './SuggestedNeedModal'
import {
    getGeneratedSteps,
    getGeneratedNeeds, //Added import to pass need generated action into props
} from '../../../actions/GeneratedAiActions'
import CustomTooltip from '../../../components/CustomTooltip'
import { Size, hp as HP, wp as WP } from '../../../asset/dimensions'
import IMAGE_SOURCE_2 from '../../../asset/image/suggestionTooltip.png'
...
const TYPE_MAP = {
    step: {
        ...
        suggested: 'Generate Suggested Steps',
    ...
    need: {
        ...
        suggested: 'Generate Suggested Needs',
...
class NewGoalView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            RANDOM_TEXT: [
                'What’s something you would be more likely to achieve if you had the right people to hold you accountable?',
                'What’s a goal so big that it excites you and scares you at the same time?',
                'What’s something you’ve been putting off that you know would make a great impact in your life if you got it done?',
        ...
        this.addGoalsToList = this.addGoalsToList.bind(this)
        this.addNeedsToList = this.addNeedsToList.bind(this)
    }
    ...
    addGoalsToList = (val) => {
        this.props.pushArray('createGoalModal', 'steps', val)
    }

    addNeedsToList = (val) => {
        this.props.pushArray('createGoalModal', 'needs', val)
    }
    ...
    renderFieldArray = (type, fields, error) => {
        ...
        console.log('TYPEEEEE', type, fields)
        ...
        return (
            <View style={styles.sectionMargin}>
                ...
                >
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            if (TYPE_MAP[type].title === 'Steps') {
                                this.props.getGeneratedSteps()
                                setTimeout(() => {
                                    this.setState({ isVisible: true })
                                }, 500)
                            } else {
                                this.props.getGeneratedNeeds()
                                setTimeout(() => {
                                    this.setState({ needsModalVisible: true })
                                }, 500)
                            }
                        }}
                    >
                ...
                <CustomTooltip
                    title={
                        type === 'step'
                            ? 'Tap here to see GoalMogul AI’s suggested Steps for achieving your goal.'
                            : 'Tap here to see GoalMogul AI’s suggested resources you might need.'
                    }
                    ...
                    tooltipConstant={
                        type === 'step'
                            ? 'generated_steps_tooltip'
                            : 'generated_needs_tooltip'
                    }
                />
                <Button
    ...
    renderNeeds = ({ fields, meta: { error, submitFailed } }) => {
        return (
            ...
            >
                <WalkableView>
                    {this.renderFieldArray('need', fields, error)}
                </WalkableView>
            </CopilotStep>
        )
    }
    ...
    selectedSteps = (steps) => {
        this.setState({ selectedSteps: steps }, () => {
            steps.map((step) => {
                this.addGoalsToList(step)
            })
        })
    }

    selectedNeeds = (needs) => {
        this.setState({ selectedNeeds: needs }, () => {
            needs.map((need) => {
                this.addNeedsToList(need)
            })
        })
    }
        ...
        return (
            ...
            >
                <SuggestedModal
                    ...
                    createGoal={this.props.handleCreateButton}
                    selectedStepsClose={this.selectedSteps}
                />
                <SuggestedNeedModal
                    isVisible={this.state.needsModalVisible}
                    onClose={(needs) =>
                        this.setState({ needsModalVisible: false })
                    }
                    createGoal={this.props.handleCreateButton}
                    selectedNeedsClose={this.selectedNeeds}
                />
                {/* <View
                    style={{
                        padding: 24,
...
export default connect(mapStateToProps, {
    ...
    searchUser,
    getGeneratedSteps,
    getGeneratedNeeds, //Add need fetching function inside props
    pushArray: arrayPush, //Add this function to append items to field array which we select from modal
})(NewGoalView)
