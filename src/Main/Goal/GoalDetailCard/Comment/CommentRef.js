...
import clarifyIcon from '../../../../asset/suggestion/clarify_white.png'
...
class CommentRef extends React.PureComponent {
    ...
    onTextLayout(e) {
        if (e.nativeEvent.lines.length > 5) {
            this.setState({ isShowMoreButtonVisible: true })
        }
    }
    ...
    renderTextContent(item) {
        const { title, content, isEditNeed } = getTextContent(item)

        const { suggestionType, _id } = item
        const defaultImage = switchDefaultImageType(suggestionType, item)
        const { imageUrl } = defaultImage
        const isClarify = suggestionType === 'Clarify'
        const { isShowMoreButtonVisible, showMore } = this.state
        const { isSelf, goalRef, onNeedPress } = this.props
        const suggestionProps = {}

        if (isClarify) {
            suggestionProps['onTextLayout'] = this.onTextLayout
        }
        return (
            <View style={{ width: '100%' }}>
                ...
                suggestionType === 'NewStep' ? null : (
                    <Text
                        ...
                        numberOfLines={isClarify ? (showMore ? 0 : 6) : 3}
                        ellipsizeMode="tail"
                        {...suggestionProps}
                    >
                        {title}
                ...
                {isClarify && isShowMoreButtonVisible && (
                    <TouchableOpacity
                        activeOpacity={0.6}
                ...
                {isClarify && isSelf && (
                    <DelayedButton
                        ...
                        onPress={() => {
                            if (isEditNeed) {
                                onNeedPress && onNeedPress()
                            } else {
                                goalRef && this.props.editGoal(goalRef)
                            }
                        }}
                        style={{
                            height: 24,
                    ...
                    >
                        ...
                        >
                            {isEditNeed ? 'Add Steps & Needs' : 'Edit My Goal'}
                        </Text>
                    </DelayedButton>
                )}
            </View>
    ...
    render() {
        const { item, userId, tab, containerStyles, owner } = this.props
        if (!item) return null
        const firstName = getFirstName(owner.name).toUpperCase()
                ...
                >
                    ...
                    >
                        {suggestionType === 'Clarify' ? (
                            ...
                            <Image
                                source={this.renderSuggestedIcon(
                                    suggestionType
                                )}
                                style={{
                                    width: 15,
                        ...
                        >
                            ...
                            {suggestionType === 'Clarify'
                                ? `${firstName} HAS A CLARIFY REQUEST`
                                : null}
                        </Text>
                    </View>
                    <DelayedButton
...
const getTextContent = (item) => {
    ...
    let clarifyText
    let isEditNeed = false
    if (suggestionType === 'Clarify') {
        if (
            suggestionText ===
            ...
            clarifyText =
                'Can you break your goal into more steps and needs so I can see how I can help?'
            isEditNeed = true
        } else if (suggestionText === 'Make your goal more specific') {
            clarifyText =
                'Can you make your goal more specific? The more specific you are, the more likely you are to achieve it. (And makes it easier for us to help!)'
        } else if (suggestionText === 'Set a clear deadline for your goal?') {
            clarifyText =
                'Maybe add a clear End Date for your goal? Accountability works wonders!'
        } else {
            clarifyText = suggestionText
        }
    }
