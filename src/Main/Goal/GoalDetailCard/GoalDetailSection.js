...
import { setSelectedGoal } from '../../../reducers/ReminderFormReducers'
...
class GoalDetailSection extends React.PureComponent {
    ...
    async componentDidMount() {
        ...
        const nudgeGoalId = await getData('goal_nudge')

        this.setState({ nudgeGoalId })
    ...
    renderUserDetail(item) {
        ...
        const caret = {
            self: {
                options: [
                    {
                        option: active_reminders?.length
                            ? 'Reminder'
                            : 'Create Reminder',
                        iconSource: BellIcon,
                        iconStyle: {
                ...
                onPress: (val) => {
                    ...
                    if (val === 'Create Reminder') {
                        this.handleGoalReminder()
                    }
                    if (val === 'Reminder') {
                        this.props.setReminderSelectedTabIndex(0)
                        Actions.push('ReminderDashboard', { goalIdToOpen: _id })
                    }
                },
    ...
    async createReminder(isCustomSelected) {
        this.setState({ isLoading: true })
        const goalDetail = this.props.goalDetail
        const goalId = goalDetail?._id

        if (goalId) {
            await this.props.createNewReminder(goalId)
            ...
            if (isCustomSelected) {
                this.props.setSelectedGoal(goalDetail)
            ...
            <>
                ...
                <ReminderPopup
                    ...
                    onClose={(isButtonClicked, isCustomSelected) => {
                        ...
                        if (isButtonClicked) {
                            this.createReminder(isCustomSelected).then()
                        }
                    }}
                    reminderData={this.props.goalDetail?.active_reminders}
                    endDate={this.props.goalDetail?.end}
                    startDate={this.props.goalDetail?.start}
                />
                ...
                <ClarifyModal
                    ...
                    name={goalDetail.owner.name}
                    gender={goalDetail.owner.gender}
                ...
                <NudgePopup
                    ...
                    noti_goalId={this.state.nudgeGoalId}
                />
