...
export class GoalDetailCardV3 extends React.Component {
    constructor(props) {
        ...
        this.onLayoutChangeForStepOrNeed = this.onLayoutChangeForStepOrNeed.bind(
            this
        )
    ...
    onLayoutChangeForStepOrNeed = (key, pageY) => {
        const onSuccess = () => {
            this.forceUpdate()
        }
        if (key === 'steps') {
            this.setState({ stepsPageY: pageY }, onSuccess)
        } else if (key === 'needs') {
            this.setState({ needsPageY: pageY }, onSuccess)
        }
    }
                ...
                return (
                    <>
                        <StepAndNeedCardV3
                            ...
                            isNudge={this.props.isNudge}
                            notification_id={notification_id}
                            onLayoutChangeForStepOrNeed={
                                this.onLayoutChangeForStepOrNeed
                            }
                        />
                    </>
                )
            ...
            case 'focusTab':
                return (
                    <CommentCard
                        ...
                        goalRef={goalDetail}
                        isSelf={this.props.isSelf}
                        onNeedPress={() => this._handleIndexChange(0)}
    ...
    render() {
        ...
        return (
            <>
                {isSelf &&
                steps.length === 0 &&
                this.state.stepsPageY &&
                !focusType ? (
                    <ProfileInfoTooltip
                        text={
                            'Steps: Your friends would love it if you added 3 or more steps to achieving your goal. The more specific you are, the more likely you will make consistent progress!'
                        }
                        width={'70%'}
                        height={125}
                        top={this.state.stepsPageY - 105}
                        infoKey={'steps_info_tooltip'}
                        image={PROFIE_INFO}
                        left={95}
                        padding={15}
                        crossTop={5}
                ...
                {isSelf &&
                needs.length === 0 &&
                this.state.needsPageY &&
                !focusType ? (
                    <ProfileInfoTooltip
                        text={
                            'Needs: Try listing 3 of more resources that could enable you to achieve this goal even faster; things that others might be able to refer or provide. This gives others the chance to delight in helping you!"'
                        }
                        width={'78%'}
                        height={170}
                        top={this.state.needsPageY + 20}
                        left={50}
                        infoKey={'needs_info_tooltip'}
                        image={MESSAGE_UI3}
                        padding={15}
                        crossTop={25}
                        crossRight={20}
...
const makeMapStateToProps = () => {
    ...
    const mapStateToProps = (state, props) => {
        ...
        const notification_id = props?.initial?.notification_id
