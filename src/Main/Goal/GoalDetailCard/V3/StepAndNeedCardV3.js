...
class StepAndNeedCardV3 extends Component {
    ...
    renderSectionTitle(item) {
        const { onLayoutChangeForStepOrNeed } = this.props
            ...
            return (
                <View
                    ...
                    onLayout={(event) => {
                        if (this.needRef) {
                            setTimeout(() => {
                                this.needRef.measure(
                                    (x, y, width, height, pageX, pageY) => {
                                        console.log('Need ref', pageY)
                                        if (onLayoutChangeForStepOrNeed) {
        ...
        return (
            <View
                ...
                onLayout={(event) => {
                    if (this.stepRef) {
                        setTimeout(() => {
                            this.stepRef.measure(
                                (x, y, width, height, pageX, pageY) => {
                                    if (onLayoutChangeForStepOrNeed) {
                                        console.log('step ref', pageY)
                                        onLayoutChangeForStepOrNeed(
                                            'steps',
                                            pageY
                                        )
