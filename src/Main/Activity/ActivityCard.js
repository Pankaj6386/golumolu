...
import SwiperTooltip from '../Common/GoalSwiperTooltip'
import SWIPER_BACKGROUND from '../../asset/image/messageUI.png'

const DEBUG_KEY = '[ UI ActivityCard ]'
const swiperText =
    'You can leave a video or voice comment by swiping left on the goal! 😃'
...
class ActivityCard extends React.PureComponent {
    ...
    handleCardOnPress = (item, props) => {
        const { goalRef, postRef, actedUponEntityType, actedWith } = item
        // console.log('ITEMM', item)
        const propsToPass = props || {}
        // if (actedUponEntityType === 'Post' && actedWith === 'Step') {
        //     return this.props.openGoalDetail(postRef, propsToPass)
        // }
    ...
    render() {
        const { item, userId, tooltipItemId } = this.props
                    ...
                    >
                        ...
                        >
                            <LikeListModal
                                ...
                                parentType={item?.goalRef ? 'Goal' : 'Post'}
                            />
                            <FloatingHearts
                            ...
                            {item._id === tooltipItemId && (
                                <SwiperTooltip
                                    title={swiperText}
                                    imageSource={SWIPER_BACKGROUND}
                                    bgStyle={{ width: 220, height: 100 }}
                                    viewStyle={{}}
                                />
                            )}
                            <View
