...
class CreateChatroomModal extends React.Component {
    ...
    componentDidMount() {
        ...
        if (!isEdit) this.props.changeModalPage(2)

        this.setState({ membersAdded: 225 }) //Set member limit by deafult to 225
        this.props.change('memberLimit', 225)
        this.props.change('membersCanAdd', true)
    }
    ...
    handleNext = () => {
        const isEdit = this.props.initializeFromState
        if (!this.props.name.trim().length && isEdit) {
            this.setState({
                ...this.state,
    ...
    renderMemberInvite() {
        ...
        const props = {
            ...
            onSelectionChange: (data) => {
                this.props.updateSelectedMembers(data)
                //Added below code to set selected members name as comma seperated group name
                if (!isEdit) {
                    let chatName = ''
                    ...
                    this.props.change('name', chatName)
                }
            },
            noHeader: true,
        }
    ...
    render() {
        ...
        return (
            <Layout style={{ flex: 1 }}>
                <ModalHeader
                    actionText={modalPageNumber === 2 ? 'Create' : ''}
                    title={titleText}
