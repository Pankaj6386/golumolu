...
import SuggestedMessages from '../../Goal/NewGoal/SuggestedMessages'
import SuggestedReplies from '../../Goal/NewGoal/SuggestionReplies'
...
class ChatRoomConversation extends React.Component {
    constructor(props) {
        ...
        this._refreshChatRoom = this._refreshChatRoom.bind(this)
    }
    ...
    async componentDidMount() {
        // initialize
        const { chatRoomId, limit, isGomoPlanner } = this.props
        if (chatRoomId === null && isGomoPlanner) {
    ...
    componentDidUpdate(prevProps) {
        ...
        // console.log('------------newChatRoomId',newChatRoomId, oldChatRoomId)
    ...
    componentWillReceiveProps(nextProps) {
        ...
        // if we change chat rooms, switch rooms accordingly and re-initialize the component
        const newChatRoomId = nextProps.chatRoomId
        const oldChatRoomId = this.props.chatRoomId
        // console.log('------------newChatRoomId',newChatRoomId, oldChatRoomId)
        if (newChatRoomId != oldChatRoomId) {
            this.props.initialLoad(newChatRoomId, this.props.limit)
            MessageStorageService.markConversationMessagesAsRead(newChatRoomId)
    ...
    handleOpenCamera = () => {
        ...
        this.props.openCamera(
            (type, result) => {
                if (type === 'video') {
                    this.props.changeMessageVideoRef(result)
    ...
    renderAccessory = (props, accessoryLocation) => {
        const { messageMediaRef, messageVoiceRef, chatRoom } = this.props
        const onRemove = () => {
            this.props.changeMessageVoiceRef(undefined)
            this._textInput.blur()
        }

        const hasGoMoReminders = chatRoom?.members.some(
            (member) =>
                member.memberRef && member.memberRef.name === 'GoMo Reminders'
        )
            ...
            return (
                <View>
                    ...
                    >
                        ...
                        >
                            {!hasGoMoReminders && this.renderGeneratedReplies()}
                            {!hasGoMoReminders &&
    ...
    renderBottomButtonSheet = () => {
        ...
        return (
            <BottomButtonsSheet
                ...
                buttons={options}
                height={sheetHeight}
                chatCameraPress={this.handleCameraOpen}
            />
        )
    }
            ...
            >
                <Layout style={styles.homeContainerStyle}>
                    ...
                    <SuggestedMessages
                        ...
                        isChat={true}
                        sendChatMessage={(generatedMessage) =>
                            this.props.sendMessage(
                                [
                                    {
                                        optionId: null,
                    ...
                    <SuggestedReplies
                        ...
                        lastMessage={this.props.messages[0]?.text}
                        isChat={true}
                        sendChatMessage={(generatedMessage) =>
                            this.props.sendMessage(
                                [
...
const mapStateToProps = (state, props) => {
    ...
    let chatRoomId = null
    let isGomoPlanner = false
    // console.log('props.chatRoom', props.chatRoomId, state.chat.gomoPlannerRef)
    if (props.chatRoomId === 'GoMoPlanner') {
        isGomoPlanner = true
        if (state.chat.gomoPlannerRef) {
            chatRoomId = state.chat.gomoPlannerRef
        }
    } else {
