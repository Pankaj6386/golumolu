...
import { getGenderPronoun } from '../../../Utils/UserUtils'
...
class NudgeCard extends React.PureComponent {
    handleNudgeCardOnPress = () => {
        ...
        if (
            ...
            return this.props.openGoalDetail(item.goalRef, '', true) //,
            // this.props.deleteSelectedNudge(_id),
            ...
            return this.props.createOrGetDirectMessage(
                sender._id,
                (err, chatRoom) => {
                    if (err || !chatRoom) {
                        return Alert.alert(
    ...
    renderNudgeContent = (item) => {
        if (
            ...
            return `sent you a nudge. ${getGenderPronoun(
                item?.sender?.gender
            )}'d like to know what your goals are too!`
        } else if (
            !item.hasResponded &&
            !item.isDeleted &&
            item.type === 'makeGoalsPublic'
